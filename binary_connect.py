...
def hard_sigmoid(x):
    return T.clip((x + 1.0) / 2.0, 0, 1)


# The binarization function
def binarization(
    W, H, binary=True, deterministic=False, stochastic=False, srng=None
):
        ...
        **kwargs
        ...
        if W_LR_scale == "Glorot":
            ...
            self.W_LR_scale = np.float32(
                1.0 / np.sqrt(1.5 / (num_inputs + num_units))
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if H == "Glorot":
            ...
            num_units = int(
                np.prod(filter_size) * num_filters
        ...
        if self.binary:
            ...
            super(Conv2DLayer, self).__init__(
                incoming, num_filters, filter_size, **kwargs
            )
...
def clipping_scaling(updates, network):
    ...
    for layer in layers:
        ...
        for param in params:
            ...
            print("H = " + str(layer.H))
            updates[param] = param + layer.W_LR_scale * (
                updates[param] - param
            )
