...
if __name__ == "__main__":
    ...
    LR_decay = (LR_fin / LR_start) ** (1.0 / num_epochs)
    ...
    # for hinge loss
    train_set.y = 2 * train_set.y - 1.0
    valid_set.y = 2 * valid_set.y - 1.0
    test_set.y = 2 * test_set.y - 1.0
    ...
    # squared hinge loss
    loss = T.mean(T.sqr(T.maximum(0.0, 1.0 - target * train_output)))
    ...
    if binary:
        ...
        updates = lasagne.updates.adam(
            loss_or_grads=W_grads, params=W, learning_rate=LR
        )
        updates = binary_connect.clipping_scaling(updates, cnn)

        # other parameters updates
        params = lasagne.layers.get_all_params(
            cnn, trainable=True, binary=False
        )
        # updates = OrderedDict(
        #     updates.items()
        #     + lasagne.updates.adam(
        #         loss_or_grads=loss, params=params, learning_rate=LR
        #     ).items()
        # )
        for key, value in lasagne.updates.adam(loss_or_grads=loss, params=params, learning_rate=LR).items():
            updates[key] = value
    ...
    test_loss = T.mean(T.sqr(T.maximum(0.0, 1.0 - target * test_output)))
    test_err = T.mean(
